...
const Nivel6 = () => {
  ...
  const verificarCSS = () => {
    ...
    if (!regex.test(css)) {
      setMensaje(
        <div className="flex items-center justify-center gap-2 text-red-600 font-semibold">
          ...
          <span>
            Usa el selector <code>li:first-child</code> correctamente con la propiedad:valor
          </span>
        </div>
      );
      return;
    ...
    if (!contieneValida) {
      setMensaje(
        <div className="flex items-center justify-center gap-2 text-red-600 font-semibold">
          <XCircle className="w-5 h-5" />
          <span>
            Al menos una propiedad debe tener un valor CSS válido - Ej: <code>color: orange;</code>
          </span>
        </div>
      );
      return;
    ...
    setTimeout(() => navigate('/juego/selectores/fin'), 3000);
  };
