  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="relative bg-white rounded-2xl shadow-lg p-6">
        ...
        {codigo && (
          <pre className="bg-gray-100 rounded-md p-4 overflow-x-auto mb-4 text-sm">
            <code>{codigo}</code>
          </pre>
        )}
        ...
        <ul className="space-y-3">
          {Object.entries(opcionesRespuesta).map(([key, label]) => (
            <li
              key={key}
              className={`flex items-center p-3 rounded-lg border-2 transition-all duration-200 ${getOptionStyle(key)}`}
              onClick={() => handleOptionClick(key)}
              onMouseEnter={() => setHoveredOption(key)}
              onMouseLeave={() => setHoveredOption(null)}
            >
              <span
                className={`w-8 h-8 flex items-center justify-center rounded-full mr-4 border-2 font-bold transition-all duration-200 ${getCircleStyle(key)}`}
              >
                {key.toUpperCase()}
              </span>
              <span className={`text-gray-700 font-medium ${opcionesOcultas.includes(key) ? "line-through" : ""}`}>
                {label}
              </span>
            </li>
          ))}
